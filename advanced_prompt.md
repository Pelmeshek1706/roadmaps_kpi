...
TASK MODE: ADVANCED MULTI-SOURCE EXTRACTION

You are given multiple course materials at once:
...
Analyze all accessible course materials together and reconstruct the course using the shared schema exactly.

SOURCE PRIORITY

Use sources with this priority when conflicts occur:
...
2. LAB_WORKS
   Use for:
   - actual implementation skills,
   - methods students must apply,
   - concrete technologies,
   - deliverables,
   - coding / testing / validation expectations.
...
4. EXTRA_MATERIALS
   Use to enrich or confirm the profile, but do not let them override official scope.

5. DOCUMENT_LINKS
   Use only if you can actually access and read them.

ACCESS AND COMPLETENESS RULES

1. If the materials are too incomplete for reliable analysis, return:

{
  ...
  "domain_scores": [],
  "source_coverage": {
    "analysis_mode": "multi_source",
    "single_document_used": false,
    "syllabus_used": false,
    "lab_works_used_count": 0,
    "lectures_used_count": 0,
    "extra_materials_used_count": 0,
    "coverage_comment": "I cannot reliably analyze the course because the provided materials are inaccessible or too incomplete."
  },
  "discipline_tags": [],
  "topic_tags": {
    "core_concepts": [],
...
2. If the materials are partial but still sufficient for bounded analysis, return "ok" with reduced confidence and explain the limitation in source_coverage.coverage_comment.

ADVANCED-MODE INTERPRETATION RULES

1. Set:
   - source_coverage.analysis_mode = "multi_source"
   - source_coverage.single_document_used = false

2. source_coverage.syllabus_used = true only if SYLLABUS_TEXT or an actually accessible official syllabus was used.

3. lab_works_used_count must count only lab items that were actually used.
4. lectures_used_count must count only lecture items that were actually used.
5. extra_materials_used_count must count only extra items that were actually used.

6. Do not let one narrow lecture or one isolated lab define the whole course if the syllabus shows broader coverage.

7. In this mode:
   - labs should strongly influence output_skills_normalized and practical_components,
   - lectures should strongly influence methods_models and theoretical_understanding,
   - syllabus should dominate scope, domain framing, and input skills.

PROCESS

1. Determine the course name.
   - Use COURSE_NAME_OVERRIDE if provided.
   - Otherwise extract the best-supported title from the materials.

2. Assess coverage.
   - Determine which sources were actually usable.
   - Reflect limitations in source_coverage.

3. Reconstruct course identity.
   - Write course_profile.one_sentence_summary and short_summary.
   - Assign discipline_positioning and course_nature.
   - Assign domain_scores.

4. Extract discipline_tags and topic_tags.
   - Keep discipline_tags broad.
   - Use topic_tags to capture conceptual, methodological, data, problem, domain, and engineering breadth.

5. Extract technology_tags.
   - Separate explicit, inferred, systems/platforms, and resource-only mentions.

6. Extract input_skills_normalized.
   - Start from explicit prerequisites in the syllabus.
   - Add only obvious inferred prerequisite skills supported by the tasks and methods.

7. Extract output_skills_normalized.
   - Use repeated lab requirements, lecture methods, project tasks, and official outcomes.
   - Prefer atomic reusable skills.

8. Build learning_outcomes.
   - Derive them from output_skills_normalized and practical_components.

9. Build practical_components.
   - Focus on what students actually implement, test, validate, design, and report.

10. Build curricular_relations.
   - Prefer normalized skill_ids.
   - Only include grounded internal relations.

11. Build student_takeaway.
   - Keep it realistic and tightly aligned with output_skills_normalized.

12. Add evidence.
   - Support major claims with short quotes or grounded paraphrases.

13. Return valid JSON only using the shared schema exactly.
